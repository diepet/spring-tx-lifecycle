...
import it.diepet.spring.tx.lifecycle.test.app.service.ProductService;
...
public class TxEventDispatcherAppTest {
	...
	public void testPersistence() {
		...
		Assert.assertTrue(stringList.get(0).startsWith("it.diepet.spring.tx.lifecycle.event.BeginEvent"));
		Assert.assertEquals("productService.add()", stringList.get(1));
		Assert.assertTrue(stringList.get(2).startsWith("it.diepet.spring.tx.lifecycle.event.CommitEvent"));
		Assert.assertTrue(stringList.get(3).startsWith("it.diepet.spring.tx.lifecycle.event.BeginEvent"));
		Assert.assertEquals("productService.findAll()", stringList.get(4));
		Assert.assertTrue(stringList.get(5).startsWith("it.diepet.spring.tx.lifecycle.event.CommitEvent"));
	}
	...
	public void testLaunchUncheckedException() {
		try {
			...
			Assert.assertTrue(stringList.get(2).startsWith("it.diepet.spring.tx.lifecycle.event.RollbackEvent"));
			return;
		}
		Assert.fail();
	}
	...
	public void testSuspendTransaction() {
		...
		Assert.assertTrue(stringList.get(2).startsWith("it.diepet.spring.tx.lifecycle.event.SuspendEvent"));
		Assert.assertEquals("warehouseService.suspendCurrentTransaction()", stringList.get(3));
		Assert.assertTrue(stringList.get(4).startsWith("it.diepet.spring.tx.lifecycle.event.ResumeEvent"));
		Assert.assertTrue(stringList.get(5).startsWith("it.diepet.spring.tx.lifecycle.event.CommitEvent"));
	}
	...
	public void testFailureInheritedTransaction() {
		try {
			...
			Assert.assertTrue(stringList.get(3).startsWith("it.diepet.spring.tx.lifecycle.event.SetRollbackOnlyEvent"));
			Assert.assertTrue(
					stringList.get(4).startsWith("it.diepet.spring.tx.lifecycle.event.failure.CommitErrorEvent"));
		}
	...
	public void testRequiresNewTransaction() {
		...
		Assert.assertTrue(stringList.get(5).startsWith("it.diepet.spring.tx.lifecycle.event.CommitEvent"));
		Assert.assertTrue(stringList.get(6).startsWith("it.diepet.spring.tx.lifecycle.event.ResumeEvent"));
		Assert.assertTrue(stringList.get(7).startsWith("it.diepet.spring.tx.lifecycle.event.CommitEvent"));
	}
}
